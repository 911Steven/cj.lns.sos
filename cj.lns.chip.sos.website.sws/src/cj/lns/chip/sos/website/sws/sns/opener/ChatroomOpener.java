...
public class ChatroomOpener implements IAppSessionOpener{
	@CjServiceInvertInjection
	@CjServiceRef(refByName="/session/openSession.html")
	OpenSessionComponent openSessionComponent;
	@Override
	public void flow(Map<String,Object>session, Map<String, String> app, Frame frame,
			Circuit circuit) throws CircuitException {
		IServiceosWebsiteModule m = ServiceosWebsiteModule.get();
		IServicewsContext sws = IServicewsContext.context(frame);
		List<String> userids = null;
		userids = getChatroomUsers((String) session.get("appId"), m);
		userids.remove(sws.owner());//移除当前的持有者，因为聊天室中不需要显示自身。
		// 根据用户标识列表查用户信息
		List<SosUser> users = getSosUsers(userids, m);
		IModuleContext ctx = m.context();
		Document doc = ctx.html("/session/OpenChatroomSession.html",
...
//			String name=String.format("%s[%s]", user.getNickName(),user.getUserCode());
			othertmsgli.select(">.box-other>.person>p").html(user.getUserCode());
	...
	public List<String> getChatroomUsers(String crid,
			IServiceosWebsiteModule m) {
		Frame f = new Frame("getUsers /device/session/chatroom peer/1.0");
		f.parameter("crid", crid);
		Circuit c = new Circuit("peer/1.0 200 OK");
		f.head(NetConstans.FRAME_HEADKEY_CIRCUIT_SYNC_TIMEOUT, "15000");//
		// 等待15秒
		try {
