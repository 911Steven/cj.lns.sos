...
@CjService(name = "/market/portletSettingPanel.html")
public class PortletSettingPanelComponent implements IComponent {
	...
	public void flow(Frame frame,Circuit circuit, IPlug plug) throws CircuitException {
		...
		Document doc=m.context().html("/market/portletSettingPanel.html", m.site().contextPath(), "utf-8");
		String url="/portal/market/getPortletFairAllCategory.service";
		...
		Elements es=doc.select(".portlet-setting>.content-panel>.list");
		Element list=es.first();
		Elements templates=list.select(".item");
		if(templates.isEmpty()){
			throw new CircuitException("503", "页面portletSettingPanel.html 中缺少列表模板");
		}
...
//			String desc=ElementGet.getJsonProp(jo.get("categoryDesc"));
			...
			newli.attr("openContent",String.format("./servicews/market/portletFair.html?category=%s", id));
