...
function setDefaultApp(swsid,appId){
	var out = ServiceosWebsiteModule.get().out();
	var frame = new Frame("setDefaultApp /sws/instance sos/1.0");
	var circuit = new Circuit("sos/1.0 200 ok");
	frame.parameter("swsid", swsid);
	frame.parameter("appId", appId);
...
exports.flow = function(frame, circuit, plug, ctx) {
	var appId=frame.parameter('appId');
	if(appId=='undefined')return;
	var sws=IServicewsContext.context(frame);
	var swsid=sws.swsid();
	setDefaultApp(swsid,appId);
}