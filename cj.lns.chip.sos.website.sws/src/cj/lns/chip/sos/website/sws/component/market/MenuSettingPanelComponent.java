...
@CjService(name = "/market/menuSettingPanel.html")
public class MenuSettingPanelComponent implements IComponent {
	...
	public void flow(Frame frame,Circuit circuit, IPlug plug) throws CircuitException {
		...
		Document doc=m.context().html("/market/menuSettingPanel.html", m.site().contextPath(), "utf-8");
		String url="/portal/market/getMenuFairAllCategory.service";
		...
		Elements es=doc.select(".menu-setting>.content-panel>.list");
		Element list=es.first();
		Elements templates=list.select(".item");
		if(templates.isEmpty()){
			throw new CircuitException("503", "页面menuSettingPanel.html 中缺少列表模板");
		}
...
//			String desc=ElementGet.getJsonProp(jo.get("categoryDesc"));
			...
			newli.attr("openContent",String.format("./servicews/market/menuFair.html?category=%s", id));
