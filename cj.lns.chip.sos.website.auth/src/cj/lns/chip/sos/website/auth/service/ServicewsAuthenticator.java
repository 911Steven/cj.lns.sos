...
@CjService(name="servicewsNumAuth")
public class ServicewsAuthenticator implements IAuthenticator{
	...
	public void authenticate(IAuthForm form) throws CircuitException {
		...
		Frame frame = new Frame(
				String.format("authServicewsId /public/auth/?account=%s&password=%s sos/1.0",
						form.get("account"), form.get("password")));
		...
		if (cnt.readableBytes() > 0) {
			...
			form.put("account", back.head("account"));
			form.put("face", new Gson().fromJson(back.head("face"), Face.class));
