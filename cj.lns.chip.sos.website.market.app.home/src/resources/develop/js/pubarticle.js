(function() {
	...
	main.delegate('.main-right>.portlet[category] > .op > input', 'keyup',
			function(e) {
				if (e.keyCode == '13') {
					...
					$.post('./swssite/develop/addArticleCategory.service', {
						name : name
					}, function(e) {
	...
	main.delegate('.portlet[category] > ul > li>span[x]', 'click', function(e) {
		...
		$.get('./swssite/develop/delArticleCategory.service', {
			id : categoryId
		}, function(e) {
			the.remove();
		}).error(function(e) {
	...
	function sendit() {
		...
		$
				.post(
						'./swssite/develop/createArticle.service',
						{
							title : title,
							content : content,
							categoryId : category,
						...
						function(data) {
							...
							var obj = eval("(" + data + ")");
							$.cj.App
									.openApp('appi://site.portal.develop.myarticle');
						}).error(function(e) {
					alert(e.responseText);
				});
...
//					v.play();
	...
	function uploadFile(e) {
		...
		fileE.on('change', function(file) {
			...
			ul.append(li);
			var url = './swssite/develop/uploadFiles.service';
			$.ajaxFileUpload({
				url : url, // 用于文件上传的服务器端请求地址
				secureuri : false, // 一般设置为false
				fileElementId : fileId,
