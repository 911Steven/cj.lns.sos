...
exports.flow = function(frame, circuit, plug, ctx) {
	...
	switch(portal){
	case 'develop':
		var call=m.site().getService('$.cj.jss.sos.develop.left');
		if(typeof call!=='undefined'&&call!=null){
			call.flow(frame,circuit,plug,ctx);
		}else{
			throw new CircuitException('404','找不到左列:left.jss.js');
		}
		break;
	case 'microblog':
		var call=m.site().getService('$.cj.jss.sos.microblog.left');
		if(typeof call!=='undefined'&&call!=null){
			call.flow(frame,circuit,plug,ctx);
		}else{
			throw new CircuitException('404','找不到左列:left.jss.js');
