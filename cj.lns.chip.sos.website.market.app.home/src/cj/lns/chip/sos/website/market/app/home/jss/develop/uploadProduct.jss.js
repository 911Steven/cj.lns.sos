...
function saveDisk(p, sws, circuit) {
	...
	var dir = fs.dir('/products/');
	if (!dir.exists()) {
		dir.mkdir('我的产品');
	}
	var file = fs.openFile('/products/' + fn);
	var writer = file.writer(0);
	writer.write(f.data());
	writer.close();
	var src = String.format("./resource/ud/%s?path=%s://products/&u=%s", fn,
			sws.swsid(), sws.owner());
	circuit.content().writeBytes(
			String.format("{'src':'%s','fn':'%s','rfn':'%s'}", src,
					f.filename(), fn).getBytes());
}