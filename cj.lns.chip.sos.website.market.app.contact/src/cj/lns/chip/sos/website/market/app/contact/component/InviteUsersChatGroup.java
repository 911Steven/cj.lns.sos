...
@CjService(name = "/inviteUserChatGroup.service")
public class InviteUsersChatGroup implements IComponent {
	...
	public void flow(Frame frame, Circuit circuit, IPlug plug)
		...
		cjql = "select {'tuple':'*'} from tuple sns.chatGroup.users java.util.HashMap where {'tuple.gid':'?(gid)','tuple.user':{$in:?(user)}}";
		IQuery<Map<String, Object>> q2 = home.createQuery(cjql);
		q2.setParameter("gid", appId);
		...
		sb.append("<script>");
		sb.append("(function(){");
		sb.append("$('div[embeds]>span[gid]>a[action]').on('click',function(){");
		sb.append("var action=$(this).attr('action');");
		sb.append("var the=$(this);");
		sb.append("var gid=the.parent('span[gid]').attr('gid');");
		sb.append("$.get('./contact/joinChatGroup.service?action='+action+'&gid='+gid,{},function(data){");
		sb.append("the.css('color','gray');");
		sb.append("});");
		sb.append("});");
		sb.append("})();");
		sb.append("</script>");
		sb.append(String.format("<div embeds>%s 邀请你加入会话室。<span gid='%s'><a href='#' action='yes' style='font-size:14px;color:blue;' >同意</a>&nbsp;&nbsp;<a href='#' action='no' style='font-size:14px;color:blue;' >拒绝</a></span></div>", sws.owner(),appId));
	...
	private void inviteSend(MessageStub msg, String[] userCodes, String appId,
		...
		f.parameter("app-id", String.format("chatGroup.%s", appId));
