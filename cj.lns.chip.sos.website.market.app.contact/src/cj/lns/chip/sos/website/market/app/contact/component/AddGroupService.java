...
@CjService(name = "/addGroup.service")
public class AddGroupService implements IComponent {

	@Override
	public void flow(Frame frame,Circuit circuit, IPlug plug) throws CircuitException {
		String gvalue = frame.parameter("groupValue");
		try {
			gvalue = URLDecoder.decode(gvalue, "utf-8");
		} catch (UnsupportedEncodingException e) {
		}
		IServiceosWebsiteModule m = ServiceosWebsiteModule.get();
		IServicewsContext sws = IServicewsContext.context(frame);
		Frame f = new Frame("addContactGroup /sws/userGroup/ sos/1.0");
		f.parameter("swsid", sws.swsid());
		f.parameter("groupName", gvalue);
