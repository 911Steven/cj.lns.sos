(function(){
	...
	folder.delegate('>.items > .file > .op > span','click',function(e){
		...
		if(typeof $(this).attr('rename')!='undefined'){
			$.post('./swssite/develop/renameFileAndDir.service?cube='+cube,{path:path,fstype:fstype},function(data){
			...
			$.post('./swssite/develop/editFileAndDir.service?cube='+cube,{path:path,fstype:fstype},function(data){
				f.remove();
			}).error(function(e){
				alert(e.responseText);
			});
	...
	pathInput.on('keyup',function(e){
		...
		var cube=$('#cube').val();
		$.post('./swssite/develop/openDir.service?cube='+cube,{path:path},function(data){
			switchDir(data);
		}).error(function(e){
			alert(e.responseText);
		});
	...
	folder.delegate(' >.items> .file >img,>.items> .file >ul>li[t]>span,>.items> .parentdir >img,>.items> .parentdir >ul>li[t]>span','click',function(e){
		...
		if(li.length>0){//只用于打开目录
			...
			var cube=$('#cube').val();
			$.post('./swssite/develop/openFileInfo.service?cube='+cube,{path:path},function(data){
	...
	fstypes.on('click',function(e){
		...
		obj.fileType=path;
		var cube=$('#cube').val();
		$.post('./swssite/develop/openSite.service?cube='+cube,{site:obj},function(data){
			//switchSite
			switchSite(data,obj);
		}).error(function(e){
			alert(e.responseText);
	...
	box.delegate('> .detail > .preview > .op > li','click',function(e){
		if(typeof $(this).attr('download')!='undefined'){
			...
			var the=$(this);
			var cube=$('#cube').val();
			$.post('./swssite/develop/loggerForDownload.service?cube='+cube,{path:path},function(data){
	...
	finder.delegate('> .folder > .toolbar > div[op]>span[tool=upload]','click',function(e){
		...
		$(this).find('#uploadfile').change(function(e){
			var cube=$('#cube').val();
			$.ajaxFileUpload({
			url : './swssite/develop/uploadFileByLocation.service?cube='+cube, // 用于文件上传的服务器端请求地址
			secureuri : false, // 一般设置为false
			fileElementId : 'uploadfile',
			dataType : 'json',// 返回值类型 一般设置为json
			data : {location:curdir},
			success : function(data, status) {
				$.post('./swssite/develop/openDir.service?cube='+cube,{path:curdir},function(data){
					switchDir(data);
				}).error(function(e){
					alert(e.responseText);
				});
	...
	finder.delegate('> .folder > .toolbar > div[op]>span[tool=newfolder]','click',function(e){
		...
		finder.find('.popup>.op>span[save]').on('click',function(e){
			...
			var cube=$('#cube').val();
			$.post('./swssite/develop/createFileAndDir.service?cube='+cube,{op:'createFolder',location:curdir,folderName:folderName,dirName:dirName},function(data){
				finder.find('.popup').remove();
				var cube=$('#cube').val();
				$.post('./swssite/develop/openDir.service?cube='+cube,{path:curdir},function(data){
					switchDir(data);
	...
	finder.delegate('> .folder > .toolbar > div[op]>span[tool=newfile]','click',function(e){
		...
		finder.find('.popup>.op>span[save]').on('click',function(e){
			...
			var cube=$('#cube').val();
			$.post('./swssite/develop/createFileAndDir.service?cube='+cube,{op:'createFile',location:curdir,fileName:fileName,content:content},function(data){
				finder.find('.popup').remove();
				$.post('./swssite/develop/openDir.service?cube='+cube,{path:curdir},function(data){
					switchDir(data);
				}).error(function(e){
