(function(){
	...
	win.delegate('.home-panel > .art-ul > .article > .body > .content > pre > .extends','click',function(e){
		...
		$.get('./cyberportApp/microblog/getIoboxContent.service',{ioboxId:id},function(data){
	...
	win.find('.home-panel > .shuoshuo > .toolbar > .buttons > li').on('click',function(e){
		if($(this).is('[pub]')){
			...
			$.post('./swssite/microblog/pubShuoShuo.service',{
		...
		if($(this).is('[cancel]')){
			...
			for(var i=0;i<files.length;i++){
				...
				$.post('./swssite/microblog/editFileAndDir.service',{
					fstype:'file',
					path:path
				},function(data){
					items.empty();
			...
			myfile.on('change', function(file) {
				...
				$.ajaxFileUpload({
					...
					success : function(data, status) {
						...
						var ul = medias.find('> .items').first();
						var videoimg="<img fn='"+fn+"' src='./swssite/microblog/img/media-blue.svg'>";
