(function() {
	$('.art-panel>.more').on('click', function(e) {
		...
		if(selected.length==0){
			cmd='./swssite/develop/myarticle.html?skip='+skip+'&limit='+limit;
		}else{
	...
	$('.art-panel>.art-ul').delegate('>.article>.body>.title','click',function(e){
		...
		var cmd='./swssite/develop/openArticle.html?id='+id;
		$.cj.App.openApp(cmd);
	});
	...
	$('.art-panel>.art-ul').delegate('> .article > .body > .op > ul > li[action]','click',function(e){
		...
		var id=$(this).parents('.article').attr('id');
		var cmd='./swssite/develop/doArticle.html?id='+id;
		var action=$(this).attr('action');
		switch(action){
		case 'del':
			break;
		default://如果是持有人则为打开，是访问者则评论或分享
			$.cj.App.openApp('./swssite/develop/openArticle.html?id='+id);
			break;
		}
